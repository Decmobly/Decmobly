...
export default function ProjectInterface({ project, techSpecs }: { project: ProjectProps, techSpecs: TechSpec[] }) {
  const images = project.images || [];
  const [selectedImage, setSelectedImage] = useState(images[0] || { id: 'default', url: '' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 flex flex-col gap-6">
            <div className="relative w-full aspect-4/3 md:aspect-16/10 bg-white border border-stone-200 overflow-hidden flex items-center justify-center">
              {/* 3. Correção: Loop tipado e Next/Image */}
              {images.map((img) => (
                <div
                    key={img.id}
                    className={cn(
                ...
                >
                    <Image
                        ...
                        alt={project.title}
                        fill
                        priority={selectedImage.id === img.id} // Prioriza a imagem selecionada
              ...
              <div className="overflow-hidden w-full">
                ...
                >
                  {images.map((img) => (
                    ...
                    >
                      {/* 4. Correção: Next/Image para Thumbnails */}
                      <Image 
                        src={img.url} 
                        alt="Thumb" 
                        fill
