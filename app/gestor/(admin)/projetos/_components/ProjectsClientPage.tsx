...
export function ProjectsClientPage({ initialProjects, categories, storageStats }: { initialProjects: ProjectWithDetails[], categories: {id: string, name: string}[], storageStats: StorageStats }) {
  ...
  const totalPercent = Math.min((storageStats.total / storageStats.limit) * 100, 100);
      ...
      <div className="md:hidden space-y-4">
        {initialProjects.map((proj) => (
            <div key={proj.id} className="bg-white border border-[#efe4cd] rounded-lg p-3 shadow-sm flex gap-3 relative overflow-hidden">
                <div className="shrink-0 w-20 h-20 rounded-md bg-[#f7f1e3] border border-[#efe4cd] overflow-hidden">
                    {proj.images[0] ? (
          ...
          <TableBody>
            {initialProjects.map((proj) => (
              <TableRow key={proj.id} className="border-[#efe4cd] hover:bg-[#fdfbf7]">
                <TableCell>
                  {proj.images[0] ? (
                    <div className="w-10 h-10 rounded overflow-hidden border border-[#efe4cd]">
                                ...
                                {currentImages.map(img => (
                                    <div key={img.id} className="relative group aspect-square rounded-md overflow-hidden border border-[#efe4cd] bg-white">
                                        ...
                                        <button 
                                            ...
                                            className="absolute top-1 right-1 bg-red-600/90 text-white rounded-full p-1 hover:bg-red-700 transition-colors shadow-sm"
                                        >
                                            <X size={12} />
                                        </button>
                                    </div>
                            ...
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                                ...
                                {newFilePreviews.map((url, idx) => (
                                    <div key={`new-${idx}`} className="relative aspect-square rounded-md overflow-hidden border-2 border-green-200 bg-white">
                                        ...
                                        <button type="button" onClick={() => removeNewFile(idx)} className="absolute top-1 right-1 bg-gray-800/80 text-white rounded-full p-1 hover:bg-black"><X size={10} /></button>
                                        <div className="absolute bottom-0 w-full bg-green-100 text-[9px] text-center text-green-800 font-bold py-0.5">ARQUIVO</div>
                                    </div>
                                ))}
                                {/* Links Externos */}
                                {externalLinks.map((link, idx) => (
                                    <div key={`link-${idx}`} className="relative aspect-square rounded-md overflow-hidden border-2 border-blue-200 bg-white">
                                        <img src={link} className="w-full h-full object-cover" onError={(e) => (e.currentTarget.style.display = 'none')} />
                                        <button type="button" onClick={() => removeExternalLink(idx)} className="absolute top-1 right-1 bg-gray-800/80 text-white rounded-full p-1 hover:bg-black"><X size={10} /></button>
                                        <div className="absolute bottom-0 w-full bg-blue-100 text-[9px] text-center text-blue-800 font-bold py-0.5">LINK</div>
                                    </div>
                                ))}
                            </div>
                        </div>
