...
{/* AJUSTE AQUI:
    Se você quiser que os cards fiquem "mais para baixo", você pode diminuir a margem negativa.
    Estava -mt-16. Se quiser menos sobreposição, mude para -mt-8 ou remova.
    Deixei -mt-16 pois agora o padding-bottom da section acima protege os botões.
*/}
<section className="relative z-20 px-4 md:px-10 lg:px-40 items-center max-w-7xl mx-auto -mt-12 lg:-mt-16 mb-8">
  {/* Mudança aqui: De Grid para Flex + Wrap + Justify Center */}
  <div className="flex flex-wrap justify-center gap-4">
    {categories.map((cat: any) => (
      <Link 
        key={cat.id} 
        href={`/ambientes/${cat.slug}`} 
        /* Lógica das larguras para simular o Grid mas permitindo centralizar:
           - Mobile (2 por linha): 50% menos metade do gap (0.5rem)
           - Desktop (4 por linha): 25% menos 3/4 do gap (0.75rem)
        */
...
{featuredProjects.length > 0 && (
  <section className="py-10 md:py-14 px-4 md:px-6 max-w-7xl mx-auto"> {/* Padding reduzido no mobile */}
    ...
    <div className="flex flex-col md:flex-row justify-between items-end mb-8 md:mb-12 border-b border-stone-100 pb-6 md:pb-8">
      ...
      {/* Botão "Ver tudo" visível apenas no Desktop aqui */}
    ...
    {/* GRID DE PROJETOS */}
    {/* Alteração Principal: grid-cols-2 no mobile com gap menor (gap-x-4) */}
        ...
        <Link key={project.id} href={`/portfolio/${project.slug}`} className="group block h-full">
          <article className="flex flex-col h-full space-y-3 md:space-y-6">
            ...
            <div className="relative aspect-4/5 md:aspect-3/4 overflow-hidden bg-stone-100 shadow-sm md:shadow-inner group-hover:shadow-md transition-all">
              ...
              {/* Categoria (Badge menor e mais ajustado no mobile) */}
            ...
            <div className="flex flex-col grow space-y-2 md:space-y-3">
              ...
              {/* Descrição: Oculta no mobile (hidden) para economizar espaço, visível no desktop (md:block) */}
    ...
    {/* Botão "Ver tudo" para Mobile (aparece embaixo do grid) */}
...
{/* PASSO A PASSO (TIMELINE MOBILE + CARDS DESKTOP) */}
...
<div className="w-auto mx-auto">
  ...
  {/* MOBILE TIMELINE (Mantido idêntico ao original) */}
  ...
  {/* DESKTOP HORIZONTAL TIMELINE (Zig-Zag) */}
  <div className="hidden md:block relative w-full h-90 my-auto">
    ...
    {/* Container dos Items (Flex para distribuir horizontalmente) */}
    <div className="flex justify-between items-center w-full h-full relative">
      {steps.map((step, idx) => {
        const isTop = idx % 2 === 0; // Passo 1 (idx 0) é Top, Passo 2 é Bottom, etc.
        
        return (
          <div key={idx} className="relative flex-1 h-full group">
            
            {/* O CARD (Posicionado Cima ou Baixo) */}
            <div className={`absolute left-0 right-0 w-[90%] mx-auto p-6 border border-[#efe4cd]/10  bg-[#5c4d3c] hover:bg-[#fdfbf7]/5 transition-all duration-300 hover:shadow-xl hover:border-[#efe4cd]/30 z-10
              ${isTop 
                ? 'bottom-1/2 mb-10 hover:-translate-y-2' // Se for item ímpar (cima)
                : 'top-1/2 mt-10 hover:translate-y-2'     // Se for item par (baixo)
              }
            `}>
            ...
            {/* A LINHA CONECTORA VERTICAL (Liga o card à bolinha central) */}
            <div className={`absolute left-1/2 -translate-x-1/2 w-px bg-[#efe4cd]/40 group-hover:bg-[#efe4cd] transition-colors duration-300
              ${isTop 
                ? 'bottom-1/2 h-10 mb-0' // Linha para cima
                : 'top-1/2 h-10 mt-0'    // Linha para baixo
              }
            `}></div>

            {/* A BOLINHA CENTRAL (O Número) */}
